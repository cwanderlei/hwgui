...
Private oMainWindow, oEdit, oPanel, oPanelE
   ...
   WriteStatus( Hwindow():GetMain(),1,"Nao Conectado" )
...
Local oModDlg, oFont
   ...
   INIT DIALOG oModDlg FROM RESOURCE "AJUDA" ON PAINT {||AboutDraw()}
   ...
   REDEFINE OWNERBUTTON OF oModDlg ID IDC_OWNB1 ON CLICK {|| EndDialog( getmodalhandle() )} ;
       FLAT TEXT "Fechar" COLOR Vcolor("0000FF") FONT oFont
...
Local hDlg := getmodalhandle()
   SetDlgItemText( hDlg, IDC_EDIT1, "localhost" )
   SetDlgItemText( hDlg, IDC_EDIT2, "root" )
   SetDlgItemText( hDlg, IDC_EDIT3, "acessobdsysimoveis" )
   SetDlgItemText( hDlg, IDC_EDIT4, "calunaty" )
   IF Empty( cServer )
...
Local hDlg := getmodalhandle()
   ...
   msginfo('cServer: '+cServer+' cUser: '+cUser+' cPassword: '+cPassword+' cDataDef: '+cDataDef)
...
Local res, stroka, poz := 0, lFirst := .T., i := 1
   ...
   IF ( res := sqlQuery( connHandle, cQuery) ) != 0
      cQuery := ""
      MsgInfo( "Falha na operacao: " + STR( res ) + "( " + sqlGetErr( connHandle ) + " )" )
